...
use app\modules\quanly\models\DiemDenVsmtSearch;
...
/**
 * DiemDenVsmtController implements the CRUD actions for DiemDenVsmt model.
 */
class DiemDenVsmtController extends \app\modules\quanly\base\QuanlyBaseController
{
    ...
    {
        $searchModel = new DiemDenVsmtSearch();
    ...
    /**
     * Displays a single DiemDenVsmt model.
     * @param integer $id
     * @return mixed
     */
    ...
    /**
     * Creates a new DiemDenVsmt model.
        ...
        if($model->load($request->post()) && $model->save() && $filedinhkem->load($request->post())){
            ...
            if($filedinhkem->fileupload != null){
                ...
                foreach($filedinhkem->fileupload as $i => $item){
                    ...
                    $file[] = 'uploads/diem-den-vsmt/'.$model->id.'/'.$item->baseName.'.'.$item->extension;
                    $path = 'uploads/diem-den-vsmt/'.$model->id.'/';
    ...
    /**
     * Updates an existing DiemDenVsmt model.
     * For ajax request will return json object
    ...
    /**
     * Finds the DiemDenVsmt model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return DiemDenVsmt the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = DiemDenVsmt::findOne($id)) !== null) {
            return $model;
        } else {
