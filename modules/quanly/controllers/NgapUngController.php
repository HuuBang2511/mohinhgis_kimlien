...
use app\modules\quanly\models\NgapUngSearch;
...
/**
 * NgapUngController implements the CRUD actions for NgapUng model.
 */
class NgapUngController extends \app\modules\quanly\base\QuanlyBaseController
{

    public $title = "Điểm ngập úng";

    /**
     * Lists all NgapUng models.
     * @return mixed
     */
    public function actionIndex()
    {
        $searchModel = new NgapUngSearch();
    ...
    /**
     * Displays a single NgapUng model.
     * @param integer $id
     * @return mixed
     */
        ...
        if($model->load($request->post()) && $model->save() && $filedinhkem->load($request->post())){
            ...
            if($filedinhkem->fileupload != null){
                ...
                foreach($filedinhkem->fileupload as $i => $item){
                    ...
                    $file[] = 'uploads/ngap-ung/'.$model->id.'/'.$item->baseName.'.'.$item->extension;
                    $path = 'uploads/ngap-ung/'.$model->id.'/';
    ...
    /**
     * Updates an existing NgapUng model.
     * For ajax request will return json object
    ...
    /**
     * Finds the NgapUng model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return NgapUng the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = NgapUng::findOne($id)) !== null) {
            return $model;
        } else {
