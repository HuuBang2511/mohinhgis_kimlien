...
/**
 * DiemDenVsmtSearch represents the model behind the search form about `app\modules\quanly\models\DiemDenVsmt`.
 */
class DiemDenVsmtSearch extends DiemDenVsmt
{
    /**
     * @inheritdoc
     */
    public function rules()
    {
        return [
            ...
            [['geom', 'dia_chi', 'thuc_trang', 'de_nghi', 'danh_gia', 'to_dan_pho', 'can_bo', 'khu_vuc', 'lat', 'long', 'created_at', 'updated_at', 'file_dinhkem'], 'safe'],
    ...
    {
        $query = DiemDenVsmt::find()->where(['status' => 1]);
        ...
        $query->andFilterWhere(['like', 'upper(geom)', mb_strtoupper($this->geom)])
            ...
            ->andFilterWhere(['like', 'upper(thuc_trang)', mb_strtoupper($this->thuc_trang)])
            ->andFilterWhere(['like', 'upper(de_nghi)', mb_strtoupper($this->de_nghi)])
